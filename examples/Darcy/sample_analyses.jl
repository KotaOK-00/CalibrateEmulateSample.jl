...
function main()
    ...
    for (case) in cases[mask]
        ...
        samples_all = zeros(Float64, T, n_params * length(methods))
        ...
        for (j, method) in enumerate(methods)
            ...
            #println("Begin MCMC - with step size ", new_step, "sample =", T)
            for i in 1:num_repeats
                ...
                chain = MarkovChainMonteCarlo.sample(mcmc, T; stepsize = sigmas[j], discard_initial = 0)
                ...
                println(size(posterior_samples)) # (n_params, 10000)
                # Back to constrained coordinates
                for i in 1:n_params
                    sum_samples[:,i] += posterior_samples[i, :]
                end
            end
            samples = sum_samples / num_repeats
            samples_all[:, (n_params * (j-1)):(n_params * j)] = samples
            #println(samples)
            means = zeros(Float64, n_params)
            covariances = zeros(Float64, n_params, n_params, T)
        end
        ##############################
        ...
        posterior_samples = reduce(vcat, [get_distribution(posterior)[name] for name in get_name(posterior)]) #samples are columns of this matrix
        println("posterior_samples: ", posterior_samples)
        =#
        #... plot etc


        alg_name = ["RWMH", "pCN", "MALA", "Barker", "∞-MALA", "∞-HMC", "∞-mMALA", "∞-mHMC", "split ∞-mMALA", "split ∞-mHMC"]
        Nalg = length(alg_name)
        dim = 100
        Ndim = 1
        ...
        PDEsolns = zeros(Nalg, Ndim)

        for i in 1:Nalg
            AP[i, 1] = rand()  # サンプル値
            spiter[i, 1] = rand() * 10  # サンプル値
            Eff[i, 1, :] = rand(3)  # サンプル値
            MinEffs[i, 1] = Eff[i, 1, 1] / spiter[i, 1]
            spdup[i, 1] = i == 1 ? 1 : MinEffs[i, 1] / MinEffs[1, 1]
            PDEsolns[i, 1] = rand() * 100  # サンプル値
        end
