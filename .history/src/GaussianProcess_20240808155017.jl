...
function _posterior(
    gp::GaussianProcess{AGPJL},
    kernel::TranformedKernel,
    input_output_pairs::PairedDataContainer{FT},
) where {FT <: AbstractFloat}
    input_values = permutedims(get_inputs(input_output_pairs), (2, 1))
    output_values = get_outputs(input_output_pairs)
    N_models = size(output_values, 1)
...
function optimize_hyperparameters!(
    ...
    for i in 1:N_models
        ...
        #optimize!(gp.models[i], args...; noise = false, kwargs...)
        opt_rbf = const_value[i] * KernelFunctions.SqExponentialKernel() ∘ ARDTransform(rbf_len[i, :])
        opt_rbf = 2.9031145778344696 * KernelFunctions.SqExponentialKernel() ∘ ARDTransform([1.9952706691900783, 3.066374123568536])
        opt_f = AbstractGPs.GP(opt_rbf)
        opt_fx = opt_f()
        post_fx = gp.models[i]
        f_orig = post_fx.gp
        data_i = post_fx.data
