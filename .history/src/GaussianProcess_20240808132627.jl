...
function build_models!(
    ...
    if gp.kernel === nothing
        ...
        const_value = 1.0
        rbf_len = ones(2)
        rbf = const_value * KernelFunctions.SqExponentialKernel() ∘ KernelFunctions.ARDTransform(rbf_len)
        kern = rbf
        println("Using default squared exponential kernel:", kern)
    else
        kern = deepcopy(gp.kernel)
...
function optimize_hyperparameters!(
    ...
    rbf_len = [1.9952706691900783, 3.066374123568536,]
    for i in 1:N_models
        # always regress with noise_learn=false; if gp was created with noise_learn=true
        # we've already explicitly added noise to the kernel

        #optimize!(gp.models[i], args...; noise = false, kwargs...)
        rbf = const_value[i] * KernelFunctions.SqExponentialKernel() ∘ KernelFunctions.ARDTransform(rbf_len)
        kern_opt = rbf
        println("optimized hyperparameters of GP: ", i)
    end
end
