...
rbf = KernelFunctions.SqExponentialKernel() ∘ ARDTransform(1 ./ rbf_len)
...
dist_sq_2 = sum(((x1 .- x2) ) .^ 2)
kval_manual_2 = exp(-0.5 * dist_sq_2 / sum(rbf_len .^ 2))

# Compare the results
println("Kernel value using ARDTransform: ", kval_julia)
println("Manually calculated kernel value: ", kval_manual)
println("Manually calculated kernel value: ", kval_manual_2)
